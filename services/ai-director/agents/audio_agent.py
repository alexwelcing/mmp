...
class AudioAgent:
    ...
    async def generate_soundscape(self, character_role: str) -> str:
        ...
        If the audio inference service is unreachable (common in local dev),
        returns a mock URL so the pipeline can continue.
        """
        ...
        try:
            ...
            logger.warning("Audio service unavailable; returning mock URL: %s", exc)
            return f"mock://audio/{character_role.lower()}.wav"
